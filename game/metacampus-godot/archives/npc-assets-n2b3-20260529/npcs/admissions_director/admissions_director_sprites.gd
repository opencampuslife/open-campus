extends RefCounted
class_name AdmissionsDirectorSprites

## admissions_director SpriteFrames 资源
## 由 NpcFactory 在运行时加载，构造完整动画集
##
## 实际文件位置（baseline/ 子目录）：
##   sprite_idle.png               → res://assets/npcs/admissions_director/baseline/sprite_idle.png
##   admissions_director_walk_down.png  等  → res://assets/npcs/admissions_director/
##
## animation_spec.json 的 naming_convention 字段仅供参考；
## 实际 idle 文件名固定为 sprite_idle.png（在 baseline/ 子目录）。

const NPC_DIR := "res://assets/npcs/admissions_director"
const SPEC_PATH := "res://assets/npcs/admissions_director/animation_spec.json"

## idle PNG 实际路径（baseline/ 子目录，固定名称）
const _IDLE_PATH := "res://assets/npcs/admissions_director/baseline/sprite_idle.png"
